...
| **⚡ 即時歷程模式**（v2.4.0） | 串流式批次重複實驗頁：跑 N 次模擬時即時看累積平均曲線往上長、95% 信賴區間以 `1/√n` 收窄、直方圖從稀疏單柱變成鐘形分配。系統會自動偵測是否已收斂並提示「可以停了」；多指標小圖網格 + 旁邊一隻貓的 SVG 動畫會隨完成的 run 一輪輪換，邊跑邊看。每個指標可選掛 **及格門檻**，所有圖會即時顯示 `P(X 達標)` 並在小圖每塊角落加 ✓/✗ 通過徽章。v2.7.0 再加上 **累積分布（CDF）檢視**：三張圖共用一條可拖曳門檻、Wilson + DKW 信賴度、反查門檻、中英對照白話結論 |
...
v2.4.0 新增專屬的 **⚡ 即時歷程模式** 頁面，把批次重複實驗從「按執行、等、看一個 CI」變成串流式、可拿來上課的體驗：跑 N 次模擬時直接看累積平均曲線往上爬、95% 信賴區間以 `1/√n` 速度收窄、直方圖從一根稀疏的柱子長成鐘形。「跑幾次才夠？」這個問題從此可以「看到」，不只能口頭相信。

**📈 累積平均曲線 + 即時 CI 帶** — `CumulativeMeanChart` 畫橘色累積平均線與淡橘色 95% 信賴區間帶，帶子隨樣本以 `1/√n` 速度收窄；右側脈動橘點代表最新一次 run；灰色虛線標出第 1 次的孤立值（讓「相信一次模擬」與「相信平均值」之間的差距被畫出來）；綠色穩定區會自動著色覆蓋累積平均近 100 次相對變化 < 1% 的區段。

**📊 即時直方圖 + KDE 平滑曲線 + 自動形狀判讀** — `LiveHistogram` 畫 20 bin 直方圖並疊一條 KDE（`utils/kde.ts`）、中位數 + P5 + P95 三條虛線、右下角自動分類分配形狀（「接近常態」、「右偏（長尾在右）」、「長尾分配（極端值偏多）」，由 `utils/distributionShape.ts` 驅動）。鐘形隨樣本累積即時浮現。

**🎯 收斂偵測器 + 慶祝動畫** — `utils/convergence.ts` 監看近 100 次累積平均，當相對變化降到 1% 以下就翻成「stable」。觸發時圖上染出綠色穩定區、提示框告訴使用者「✓ 結果已穩定，可以停了！」並播一次性 `ConfettiBurst`。閾值與視窗寬度直接寫在提示文字裡，讓它變成可教學的訊號而不是黑盒燈號。

**🐱 跟著完成 run 輪播的 SVG 咖啡廳動畫** — 批次跑的同時，左半邊會播跟一般回放頁同一套貓咪動畫，但每跑完一次 run 就換到那次的事件流。下方 `RunThumbnailStrip` 列出所有完成的 run，點任一格就把動畫切到那次。輪換有 gating，只在使用者目前那次播完或暫停時才換，不會在播到一半被硬抽掉。

**📐 多指標小圖** — 一格格小型累積平均圖（預設 6 個指標、可設定、會記住），點任一格進入該指標的完整圖 + 直方圖檢視。21 個 KPI 可選：等位 / 等餐的平均與百分位、總停留、放棄率、三種利用率、貓咪互動率、貓咪福祉指數、顧客滿意度等等。

**▶️ 串流 runner，可暫停 / 停止 / 部分提交** — `useLiveRunner` 一次跑一次模擬並把結果丟進 Zustand `liveBatchStore`。runner 有 pause / resume / stop，單次 run 出錯不會中斷整個 batch，使用者中途按停止時也會把已完成的部分送進 commit step — 50 次的 batch 在第 27 次喊停，仍能在 Results 頁產生有效的 27-run 摘要。

**🔁 重播曲線** — 「重播曲線」按鈕會把所有累積平均 / 直方圖 bin 從 `n=1` 重畫到結尾，動畫呈現成長過程，方便跑完之後拿給觀眾看收斂故事。

**💡 可發現性引導** — 模擬設定頁在批次與敏感度都關著時，會在開關上方出現橘色虛線提示卡：「跑 1 次 → 一個點（不知道準不準）。跑 20 次 → 一條曲線 + CI 帶（看穩定性）」。模擬回放頁在使用者跑完單次模擬後也會跳出一次性 `BatchPromoBanner`，提供一鍵「🚀 立刻跑 20 次批次」CTA。兩者在使用者跑出第一個批次之後永久隱藏。

**📚 N 與 n 的記號詞彙表** — 兩個新的 hover 詞條（`bigN` / `lowerN`），讓所有圖表文字可以採用統計慣例 — **大寫 N** = 你計劃要跑的總數、**小寫 n** = 目前已完成的數，第一次看圖的人也不會混淆。

**🎯 及格門檻 + 即時通過機率** — 每個指標圖表都支援掛上可選的及格／失敗門檻（例如 `catWelfareScore ≥ 3.5`、`abandonRate ≤ 15%`）。累積平均圖會畫一條橘色虛線通過線，直方圖會在通過側標上 `≥ X: NN%` / `≤ X: NN%`，小圖網格每塊角落多一顆綠色 ✓ / 灰色 ✗ 通過徽章，一眼掃過五個指標就知道哪幾個還沒過關。內建灌入 5 個指導教授真的會問的預設門檻（`catWelfareScore`、`customerSatisfactionScore`、`avgWaitForSeat`、`abandonRate`、`seatUtilization`）；網格上方新增可摺疊的 **及格門檻總覽**（`GlobalThresholdBar`），可以一次設定或清除所有可見指標的門檻，附 `≥` / `≤` 方向切換與每指標的 step / 上下界檢查。通過機率 `P(X 達標)` 在每次 run 完成的當下立即重算，不用重跑模擬，由 `utils/exceedance.ts` 的純函式 `calcExceedProb()` 統一驅動，直方圖標籤與小圖徽章共用同一個算式。

**📈 累積分布（CDF）+ 達標機率分析（v2.7.0）** — 指標放大檢視新增一張 `CdfChart`，讓直方圖（看頻率）、KDE 密度（看形狀）、CDF（看累積機率）三張圖共用同一條 **可拖曳的門檻線**：在任一張圖拖門檻，其他兩張與所有讀數即時連動。CDF 圖畫經驗階梯 `F_N(x)`、可選的 **平滑 CDF**（沿用與 KDE 密度相同的 Silverman 頻寬）、以及可選的 **DKW 95% 信賴帶**（樣本少時自然變寬，誠實反映不確定性）。讀數卡顯示達標機率與其 **Wilson 95% 區間**、`F(t)` 與 `1 - F(t)`、對目標達標率的 PASS / FAIL 判定，並自動產生一句數字即時帶入的 **中英對照結論句**。**反查門檻**回答反過來的問題（「想要 75% 達標，標準最多能放到哪？」）並畫出建議門檻線。每張圖與讀數卡都附常駐的白話說明，沒有統計背景也看得懂。計算層放在不碰 UI、可單元測試的 `utils/cdf.ts`（本專案第一套 **Vitest** 測試）。
...
**🎓 驗證方法學章節** — 頁首新增可摺疊卡片，引用 Sargent (2013)、Kleijnen (1995)、Rubner (2000)、Agresti (2013)、Wilson (1927) 作為合成分數的統計學依據。重點橙色 callout 指出 **Hirsch 2025 論文本身就用 χ² goodness-of-fit**（χ²(2) = 1234.2、χ²(16) = 1113.2 等四個檢定，全部 p < 0.001 + Bonferroni 校正），我們用的跟論文同源，不是自己新發明的。3×3 互補性矩陣列出每個指標的擅長偵測、盲區、互補對，80 分閾值也用 Agresti 的臨界值 + Sargent 的 face validity ≥ 0.8 慣例背書。

**📊 分類別指標展開面板** — 三個 sub-score 卡片（χ² / KS / KL）現在都可以點開。點 χ² → 展開 9 列殘差貢獻表（O、E、O−E、標準化殘差、貢獻 (O−E)²/E、對總卡方的佔比用橫向條形圖視覺化），一眼看出哪一類貢獻了最多卡方。點 KS → 展開 600×200 SVG CDF 疊圖，紅色垂直線標最大差距的確切位置。點 KL → 展開雙向條形圖（橘色過度投入、藍色投入不足）+ `exp(−KL) × 100%` 保真度解讀。三張面板都誠實揭露「嚴格臨界值會拒絕」和「soft-cap 設計容忍雜訊」的取捨，不隱藏。

**📜 實證資料審計軌跡** — 比對基準卡片下方新增可摺疊卡片。`benchmarks.ts` 從 `Record<string, number>` 擴充為 `Record<string, CategoryBenchmark>`，每類帶 `{proportion, n, total, wilsonCI95, source: {figureOrTable, page?, note?}}`。卡片呈現兩張表（行為分布 + 垂直層級）列出每類的實證%、原始 n、95% Wilson 信賴區間、模擬%、✓或⚠ 是否在 CI 內、以及論文位置。數值從 Hirsch 2025 PDF 直接抽取：總 scan 12,505 次（107 observations × 70 天 × 27 貓 × 227 小時）、in-lounge n = 8547、vertical 各級數字 1560 / 2778 / 4209 論文直接給出。

**🧪 可教學公式** — 頁面上六個公式（合成分數、soft-cap 映射、Pearson χ²、KS D、KL 相對熵、Wilson CI）全部改用 `InteractiveFormula` 這個可點擊 chip 教學元件（跟 Results / Settings 頁面同一套）。點任何符號（p̂、n、z、χ²、Oᵢ、Eᵢ、(Oᵢ−Eᵢ)²、Σ、k、F_sim、F_emp、D_KL、P、Q、pᵢ、qᵢ、log-ratio 等）會展開卡片顯示它的名稱、白話說明、單位、以及代入本次模擬實際數字的具體例子。每個公式也附 🔍 永遠可見的一行 hint，以及可選的「更多說明 ▼」深度解釋。

**📖 術語詞彙庫擴充（+19 個）** — `goodness-of-fit`、`Kolmogorov-Smirnov`、`Kullback-Leibler divergence`、`Wilson score`、`CDF`、`p-value`、`critical value`、`degrees of freedom`、`standardized residual`、`residual`、`nats`、`Monte Carlo`、`Bonferroni correction`、`face validity`、`multi-metric composite`、`soft-cap`、`Type I error`、`outlier`、`V&V`。驗證頁面的所有說明文字（方法學、解讀 callout、provenance 前言）都跑過 `renderWithTerms`，這些術語滑鼠 hover 或 tab focus 會彈出 popover，中英雙語。

語氣混合：引用他人論文時用學術口吻（「Sargent 2013 建議...」），自己的設計選擇則第一人稱（「我挑了三個互補指標是因為...」），跟 app 既有的文案風格一致。
...
**📐 Hirsch 2025 Figure 4 基準值對齊 + `hirsch-paper` 情境預設** — 模擬器的 `HIRSCH_VERTICAL_BASE` 現在精準對上驗證器的目標值（0.182 / 0.325 / 0.492，來自 Figure 4 的「在咖啡廳內」小計，n = 8547），不再是手調近似值。新增「**論文樣本（Hirsch 2025）**」情境預設，一鍵載入完整的 35 座 / 27 貓 / 720 分鐘配置，讓重現論文分數變成一次點擊。原本三個預設（平日白天 / 假日尖峰 / 貓咪午睡）的 `seatCount` 從 10 升到 35，以配合新平面圖的店面尺度。驗證頁面搭配這個預設加了**方法學章節**，引用 Sargent (2013) / Kleijnen (1995) / Rubner (2000) / Agresti (2013) / Wilson (1927) 作為 χ² / KS / KL 合成分數的統計學依據，並新增**分類別審計表**顯示每個基準值的原始 n、95% Wilson 信賴區間、論文 Figure / Table 位置，讓「87 / 100」這個分數可以一路追溯回論文來源。
