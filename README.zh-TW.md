...
| **⚡ 即時歷程模式**（v2.4.0） | 串流式批次重複實驗頁：跑 N 次模擬時即時看累積平均曲線往上長、95% 信賴區間以 `1/√n` 收窄、直方圖從稀疏單柱變成鐘形分配。系統會自動偵測是否已收斂並提示「可以停了」；多指標小圖網格 + 旁邊一隻貓的 SVG 動畫會隨完成的 run 一輪輪換，邊跑邊看。每個指標可選掛 **及格門檻**，所有圖會即時顯示 `P(X 達標)` 並在小圖每塊角落加 ✓/✗ 通過徽章 |
