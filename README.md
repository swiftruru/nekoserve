...
| **⚡ Live Mode** (v2.4.0) | Streaming batch-replication page: launch N runs and watch the cumulative-mean curve climb, the 95% CI band tighten as `1/√n`, and the histogram morph from one bar into a bell. Auto-detects "stable" and tells you when you can stop. Multi-metric small-multiples grid + a live SVG café animation that cycles through every completed run as it lands. Per-metric **pass bars** with a live `P(X passes)` overlay on every chart, plus pass/fail chips on every small-multiples tile. v2.7.0 adds a **cumulative-distribution (CDF) view** with a draggable threshold shared across all three views, Wilson + DKW confidence, an inverse threshold lookup, and a bilingual plain-language conclusion. |
...
**🎯 Pass bars + live exceedance probability** — every metric chart now accepts an optional pass/fail bar (e.g. `catWelfareScore ≥ 3.5`, `abandonRate ≤ 15%`). The cumulative-mean chart draws an orange dashed pass-bar line; the histogram labels the passing side with `≥ X: NN%` / `≤ X: NN%`; every small-multiples tile gets a green ✓ / gray ✗ chip in the corner so you can scan five metrics at a glance and spot which ones are still failing. Five committee-grade defaults pre-seeded (`catWelfareScore`, `customerSatisfactionScore`, `avgWaitForSeat`, `abandonRate`, `seatUtilization`); a collapsible **Pass-bar overview** card above the grid (`GlobalThresholdBar`) lets you set or clear bars for every visible metric in one place, with direction toggle (`≥` vs `≤`) and per-metric step / bounds enforcement. The pass fraction `P(X passes)` recomputes live on every run completion (no re-run needed), driven by a pure `calcExceedProb()` helper in `utils/exceedance.ts` that both the histogram label and the small-multiples chips read off the same source of truth.

**📈 Cumulative distribution (CDF) + attainment-probability analysis (v2.7.0)** -- a new `CdfChart` joins the metric detail page so the histogram (how often), the KDE density (what shape), and the CDF (how much probability has accumulated) share one **draggable threshold line**: drag the bar on any view and the others plus every readout update live. The CDF view draws the empirical staircase `F_N(x)`, an optional **smoothed CDF** that reuses the same Silverman bandwidth as the KDE density, and an optional **DKW 95% band** that widens honestly when N is small. A readout card shows the attainment probability with a **Wilson 95% interval**, `F(t)` and `1 - F(t)`, a PASS / FAIL verdict against a target, and an auto-generated **bilingual conclusion sentence** with every number driven live. An **inverse lookup** answers the reverse question ("how loose can my standard be if I want 75% to pass?") and draws the suggested threshold. Every chart and the readout carry an always-visible plain-language explanation, so a reader with no statistics background can follow along. The math lives in a framework-free, unit-tested `utils/cdf.ts` (the project's first **Vitest** suite).
